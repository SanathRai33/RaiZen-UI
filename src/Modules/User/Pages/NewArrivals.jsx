...
import '../CSS/Home.css';
import { Container, Grid, Card, CardMedia, CardContent, Typography, CardActions, Box } from "@mui/material";
                        ...
                        newProducts.length === 0 ? (
                            ...
                            <Grid item xs={12} sm={6} md={4} px={6} sx={{ display: 'flex', alignItems: 'center', flexWrap: "nowrap", gap: 5, minWidth: "100%", overflowX: "scroll", scrollbarWidth: 'none', '&::-webkit-scrollbar': { display: 'none', }, }}>
                                {newProducts.map((product) => (
                                    <Card key={product.id} sx={{ minWidth: 350, maxWidth: 350, borderRadius: '16px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', cursor: 'pointer' }} onClick={() => navigate(`/product/${product._id}`)}>
